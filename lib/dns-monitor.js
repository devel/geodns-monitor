...
var _process_json = function(c) {
    ...
    try {
        data = JSON.parse(c.json)
    }
    catch(ex) {
        console.log("Invalid JSON data:", ex, "(" + c.json + ")");
        return;
    }
    delete c.json;
    ...
    if (data.up) {
        c.uptime = parseInt(data.up, 10);
        c.uptime_p = timeago(new Date() - (c.uptime * 1000));
    }

    c.status = "";

    events.emit('status', c);
...
var _check_server = function(ip) {
    ...
    if (!c.ws_errors) { c.ws_errors = 0; }

    var ws_next_check = (c.ws_errors * c.ws_errors) * 1000;
    if (ws_next_check > 120000) {
        ws_next_check = 120000;
    }
    // console.log(ip, "next in", (c.ws_last_check + ws_next_check - now)/1000, "seconds; now: ", now, "// next: ", c.ws_last_check + ws_next_check);
    if (!c.ws_last_check || (now > ( c.ws_last_check + ws_next_check))) {
